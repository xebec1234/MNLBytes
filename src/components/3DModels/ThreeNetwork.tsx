...
const ThreeNetwork = () => {
  ...
  React.useEffect(() => {
    ...
    const nodeGeometry = new THREE.SphereGeometry(0.5, 32, 32); // Increased node size
    ...
    nodePositions.forEach((position, index) => {
      ...
      if (index === 0) node.scale.setScalar(2); // Increased central node size
      nodes.push(node);
    ...
    camera.position.y = 4; // Adjusted camera position
    camera.lookAt(0, 0, 0);
    ...
    const animate = () => {
      ...
      nodes.forEach((node, i) => {
        const scale = i === 0 ? 2 : 1; // Increased scale for the center node
